  ...
  return (
    <aside className="w-72 h-full bg-zinc-900/95 backdrop-blur-sm border-r border-zinc-800 flex flex-col pt-14 md:relative">
        ...
        {chats.map((chat) => (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p className="font-medium text-base text-white truncate">{chat.name}</p>
              <p className="text-xs text-zinc-400 truncate">
                {chat.history[chat.history.length - 1]?.text || "No messages"}
              </p>
            </div>
          </button>
        ))}
      ...
      <div className="p-3 border-t border-zinc-800">
        <button
          onClick={onNewChat}
          className="w-full flex items-center justify-center gap-2 h-12 rounded-xl bg-zinc-800 hover:bg-zinc-700 border border-zinc-700 text-white transition-all duration-200 hover:shadow-lg hover:shadow-zinc-900/20 group"
        >
          <svg
